...
REGISTER_COMPONENT(
    ...
    [](const std::map<std::string, std::shared_ptr<FieldValue>>& fields) {
        auto musicFile = std::get<std::string>(
            *fields.at(constants::MUSICFILE_FIELD));
        ...
        if (initialStateStr == constants::PLAYING_FIELD)
            initialState = ecs::PLAYING;
        else if (initialStateStr == constants::PAUSED_FIELD)
            initialState = ecs::PAUSED;
        else if (initialStateStr == constants::CHANGING_FIELD)
            initialState = ecs::CHANGING;
        else if (initialStateStr == constants::STOPPED_FIELD)
            initialState = ecs::STOPPED;

        return std::make_shared<ecs::MusicComponent>(
            musicFile, initialState, volume, loop);
    }
)
