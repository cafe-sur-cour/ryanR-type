...
SpatialGrid::SpatialGrid(float worldWidth, float worldHeight, float cellSize)
    : _worldWidth(worldWidth), _worldHeight(worldHeight), _cellSize(cellSize) {
    _numCols = static_cast<size_t>(std::ceil(_worldWidth / _cellSize)) + 2;
    _numRows = static_cast<size_t>(std::ceil(_worldHeight / _cellSize)) + 2;
...
size_t SpatialGrid::getCellIndex(float x, float y) const {
    int col = static_cast<int>(std::floor((x + _cellSize) / _cellSize));
    int row = static_cast<int>(std::floor((y + _cellSize) / _cellSize));
...
std::vector<size_t> SpatialGrid::getCellIndices(const math::FRect& bounds) const {
    ...
    int minCol = static_cast<int>(std::floor((left + _cellSize) / _cellSize));
    int maxCol = static_cast<int>(std::floor((right + _cellSize) / _cellSize));
    int minRow = static_cast<int>(std::floor((top + _cellSize) / _cellSize));
    int maxRow = static_cast<int>(std::floor((bottom + _cellSize) / _cellSize));
