...
void rserv::HttpServer::loadEnv() {
    ...
    if (!envFile.is_open()) {
        std::cerr << "Warning: .env file not found in server/http/. Using default password." << std::endl;
    ...
    envFile.close();
    if (_password.empty()) {
        std::cerr << "Warning: HTTP_PASSWORD not found in .env. Using default password." << std::endl;
        _password = constants::DEFAULT_HTTP_PASSWORD;
    }
}
