...
bool rserv::Server::processConnections(std::pair<asio::ip::udp::endpoint,
    ...
    debug::Debug::printDebug(this->_config->getIsDebug(), "[SERVER] Set client " + std::to_string(this->_nextClientId) + " ready to false", debug::debugType::NETWORK, debug::debugLevel::INFO);
    this->canStartPacket();
    this->_nextClientId++;
    return true;
}
