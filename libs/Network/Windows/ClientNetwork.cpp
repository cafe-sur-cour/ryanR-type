...
void WindowsNetwork::init(uint16_t port, const std::string host) {
    ...
    if (!asioLoop) {
        throw std::runtime_error("[CLIENT NETWORK] EventLoop is not an AsioEventLoop");
    }

    if (!this->_socket) {
        this->_socket = std::make_shared<asio::ip::udp::socket>(asioLoop->getIoContext());
    }
...
bool WindowsNetwork::sendTo(const NetworkEndpoint& endpoint, std::vector<uint8_t> packet) {
    try {
...
bool WindowsNetwork::broadcast(const std::vector<NetworkEndpoint>& endpoints,
...
std::pair<NetworkEndpoint, std::vector<uint8_t>> WindowsNetwork::receiveAny() {
    return {};
}
