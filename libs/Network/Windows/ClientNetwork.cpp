...
bool ClientNetwork::sendTo(const NetworkEndpoint& endpoint, std::vector<uint8_t> packet) {
    try {
...
bool ClientNetwork::broadcast(const std::vector<NetworkEndpoint>& endpoints,
...
std::pair<NetworkEndpoint, std::vector<uint8_t>> ClientNetwork::receiveAny() {
    return {};
}
