...
std::shared_ptr<IPacketManager> WindowsNetwork::receiveFrom(const int &connectionId) {
    (void)connectionId;
    return nullptr;
}
...
IPacketManager &WindowsNetwork::receiveData(const IBuffer &buffer, size_t size) const {
    (void)buffer;
    (void)size;
