...
TEST_F(PacketTest, PackBodyProducesCorrectDisconnectionBody) {
    ...
    std::vector<uint64_t> payload = {0x04};
    std::vector<uint8_t> body = packet.pack(1, 1, DISCONNECTION_PACKET, payload);

    EXPECT_GT(body.size(), 13);
    EXPECT_EQ(body[0], 0x93);
    EXPECT_EQ(body[6], DISCONNECTION_PACKET);
}
...
TEST_F(PacketTest, PackBodyWithEmptyPayload) {
    ...
    std::vector<uint8_t> body = packet.pack(1, 1, ACCEPTATION_PACKET, payload);
...
TEST_F(PacketTest, PackBodyWithUnknownPacketType) {
    ...
    std::vector<uint64_t> payload = {0x04};
    std::vector<uint8_t> body = packet.pack(1, 1, 0xFF, payload);
...
TEST_F(PacketTest, PackBodyForEventPacket) {
    ...
    std::vector<uint64_t> payload = {0x01, 0x02};
    std::vector<uint8_t> body = packet.pack(1, 1, EVENT_PACKET, payload);

    EXPECT_GT(body.size(), 13);
    EXPECT_EQ(body[0], 0x93);
    EXPECT_EQ(body[6], EVENT_PACKET);
}

TEST_F(PacketTest, PackBodyForAcceptationPacket) {
    pm::PacketManager packet(1);

    std::vector<uint64_t> payload = {0x2A};
    std::vector<uint8_t> body = packet.pack(1, 1, ACCEPTATION_PACKET, payload);

    EXPECT_GT(body.size(), 13);
    EXPECT_EQ(body[0], 0x93);
    EXPECT_EQ(body[6], ACCEPTATION_PACKET);
}
...
TEST_F(PacketTest, PackAndUnpackAcceptationBodyRoundTrip) {
    ...
    std::vector<uint8_t> body = packet1.pack(1, 1, ACCEPTATION_PACKET, payload);
...
TEST_F(PacketTest, PackAndUnpackEventBodyRoundTrip) {
    pm::PacketManager packet1(1);
    std::vector<uint64_t> payload = {7, 8};
    std::vector<uint8_t> body = packet1.pack(1, 1, EVENT_PACKET, payload);
...
TEST_F(PacketTest, ResetAfterPackAndUnpack) {
    ...
    std::vector<uint64_t> payload = {50};
    packet.pack(1, 1, ACCEPTATION_PACKET, payload);
